...
class Imagine(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.exponential_images_save = exponential_images_save
    ...
    def train_step(self, epoch, i, images_idx_to_save, pbar=None):
        total_loss = 0
    ...
    def forward(self):
        ...
        images_idx_to_save = [x for x in sorted(set([int(1.05 ** x) for x in range(200)])) if x <= self.iteration_stop]
        image_pbar = tqdm(total=len(images_idx_to_save), desc='image update', position=2, leave=True)
        ...
        for epoch in trange(self.epochs, desc = '      epochs', position=0, leave=True):
            ...
            for count,i in enumerate(pbar):
                ...
                out, loss = self.train_step(epoch, i, images_idx_to_save,image_pbar)
                pbar.set_description(f'loss: {loss.item():04.2f}')

                if terminate:
                    print('detecting keyboard interrupt, gracefully exiting')
                    return
                if count>self.iteration_stop:
                    print('detecting keyboard interrupt, gracefully exiting')
                    return 
